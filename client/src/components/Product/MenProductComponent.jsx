...
export default function MenProductComponent() {
  ...
  // Fetching Men product with firebase
  useEffect(() => {
    // Function to fetch data
    async function fetchProduct() {
      try {
        ...
        if (!snapshot.exists()) {
          ...
          const data = Object.values(snapshot.val()); // Convert to array

          // Get Men's shoes
          const menShoes = data.filter(
          ...
          for (let i = 0; i < menShoes.length; i += PER_PAGE) {
            result.push(menShoes.slice(i, i + PER_PAGE)); // Slice data per page
          }
